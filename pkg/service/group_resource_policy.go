...
func (s *groupResourcePolicyService) Alter(
	...
	if len(createdGroupResourcePolicies) > 0 {
		err := s.manager.BulkInsertWithTx(tx, createdGroupResourcePolicies)
		if err != nil {
			return errorWrapf(err, "manager.BulkInsertWithTx fail policies=`%v`", createdGroupResourcePolicies)
		}
	}
	// 删
	if len(deletedPKs) > 0 {
