...
type GroupService interface {
	...
	ListPagingGroupMember(groupPK, limit, offset int64, isSorted bool, sortType string) ([]types.GroupMember, error)
...
func (l *groupService) ListPagingGroupMember(groupPK, limit, offset int64, isSorted bool, sortType string) ([]types.GroupMember, error) {
	daoRelations, err := l.manager.ListPagingGroupMember(groupPK, limit, offset, isSorted, sortType)
	if err != nil {
		return nil, errorx.Wrapf(err, GroupSVC,
			"ListPagingGroupMember", "manager.ListPagingGroupMember groupPK=`%d`, limit=`%d`, offset=`%d`, isSorted=`%v`, sortType=`%s`",
			groupPK, limit, offset, isSorted, sortType)
	}
